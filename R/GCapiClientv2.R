...
GCapiClientV2 <- R6Class(
  "GCapiClientV2",
  public = list(
    rest_url_v1 = "https://ciapi.cityindex.com/TradingAPI",
    ...
    initialize = function(username, password, appkey, proxies = NULL) {
      ...
      response <- postForm(
        paste0(self$rest_url_v2, '/session'),
        .opts = list(
          httpheader = headers,
    ...
    get_account_info = function(get = NULL) {
      response <- getURL(paste0(self$rest_url_v2, '/UserAccount/ClientAndTradingAccount'), curl = self$session)
      resp <- fromJSON(response)
      ...
      self$client_account_id <-resp$tradingAccounts$clientAccountId
    ...
    get_market_info = function(market_name, get = NULL) {
      ...
      response <- getURL(paste0(self$rest_url_v1, '/cfd/markets?marketName=', URLencode(market_name), '&ClientAccountId=',self$client_account_id), curl = self$session)
      resp <- fromJSON(response)
    ...
    get_prices = function(market_id, num_ticks, from_ts, to_ts, price_type = "MID") {
      ...
      endpoint <- paste0('/market/', market_id, '/tickhistorybetween?fromTimeStampUTC=', from_ts, '&toTimeStampUTC=', to_ts, '&maxResults=', num_ticks, '&priceType=',toupper(price_type), '&ClientAccountId=',self$client_account_id)
      response <- getURL(paste0(self$rest_url_v1, endpoint), curl = self$session)
      resp <- fromJSON(response)
    ...
    get_ohlc = function(market_id = NULL, num_ticks = NULL, interval = "HOUR", span = 1, from_ts, to_ts) {
      endpoint <- paste0('/market/', market_id, '/barhistorybetween?interval=', interval, '&span=', span, '&fromTimeStampUTC=', from_ts, '&toTimeStampUTC=', to_ts, '&maxResults=', num_ticks, '&ClientAccountId=',self$client_account_id)
      response <- getURL(paste0(self$rest_url_v1, endpoint), curl = self$session)
      resp <- fromJSON(response)
    ...
    trade_order = function(quantity, offer_price, bid_price, direction, trading_acc_id, market_id, market_name, stop_loss = NULL, take_profit = NULL, trigger_price = NULL, close = FALSE, order_id = NULL, tolerance = NULL) {
      ...
      order_details <- list(
        ...
        PositionMethodId = 1,
        isTrade = TRUE,
        ClientAccountId=self$client_account_id # to be checked!!!!
      )
      ...
      response <- postForm(
        paste0(self$rest_url_v1, endpoint),
        .opts = list(
          httpheader = headers,
          postfields = body,
          ssl.verifypeer = TRUE # Enable SSL verification
    ...
    get_trade_history = function(trading_account_id = self$trading_account_id, from = NULL, max_results = 100) {
      ...
      endpoint <- paste0(self$rest_url_v1, '/order/tradehistory?TradingAccountId=', trading_account_id, '&maxResults=', max_results)
      if (!is.null(from)) {
